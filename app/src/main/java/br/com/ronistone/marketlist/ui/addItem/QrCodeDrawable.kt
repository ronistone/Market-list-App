...
class QrCodeViewModel(barcode: Barcode) {
    ...
    init {
        when (barcode.valueType) {
            Barcode.FORMAT_EAN_13, Barcode.TYPE_PRODUCT, Barcode.FORMAT_EAN_8 -> {
                qrContent = "BarCode: ${barcode.rawValue.toString()}"
            }
            else -> {
                qrContent = "Unsupported data type: ${barcode.rawValue.toString()} -- ${barcode.valueType}"
            }
        }
    }
}
